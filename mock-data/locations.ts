...
const getRandomTags = () => {
  ...
  // Use a stable sort for tags to ensure order is consistent
  const shuffled = [...allTags].sort((a, b) => {
    // A simple deterministic shuffle based on string comparison
...
const createLocation = (
  ...
  rating,
  tags: getRandomTags(),
  createdAt: `2024-${String(Math.floor(random() * 12) + 1).padStart(
    2,
    "0"
  )}-${String(Math.floor(random() * 28) + 1).padStart(2, "0")}`,
  isFavorite,
  visitCount: Math.floor(random() * 100) + 1,
  lastVisited:
    random() > 0.3
      ? `2024-12-${String(Math.floor(random() * 28) + 1).padStart(2, "0")}`
      : undefined,
});
